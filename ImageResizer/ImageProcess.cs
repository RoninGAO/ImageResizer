    ...
    public class ImageProcess
    {
        ...
        public async Task ResizeImages(string sourcePath, string destPath, double scale)
        {
            ...
            foreach (var filePath in allFiles)
            {
                ...
                tasks.Add(Task.Run(() =>
                {
                    Console.WriteLine($"Thread ID:{Thread.CurrentThread.ManagedThreadId}-{Path.GetFileName(filePath)} start");

                    Stopwatch sw_process = new Stopwatch();
                    sw_process.Start();
                    ...
                    Console.WriteLine($"檔案{Path.GetFileName(filePath)},花費時間: {sw_process.ElapsedMilliseconds} ms");
                    Console.WriteLine($"Thread ID:{Thread.CurrentThread.ManagedThreadId}-{Path.GetFileName(filePath)} end");
